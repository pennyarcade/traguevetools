...
def _buildAuthJWT(config):
    ...
    if encoding == 'base64std':  # with + and /
        ...
        raise Exception(
            'Invalid auth_jwt_encoding in config: "%s" (should be "clear", "base64std" or "base64url")' % encoding)
...
class SessionCache:
    ...
    def __init__(self, timeout=3600):
        ...
        log.info(
            'Session timeout is %d seconds. Checking for expired sessions every %d seconds. ' % (timeout, interval))
